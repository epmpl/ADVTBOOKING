...
function search_data() {
    ...
    Execopoup.bindgrid(compcode, execcode, callback_data);
...
function opengrid() {
        ...
        if (document.getElementById("hdsviewgrid").innerHTML == "") {
            ...
            createhtml += "<input  type='hidden' id='hdnexecslab_no_" + klen + "'>";
...
function creatnewrow() {
    ...
    if (brow_ver >= "9.0" || browser !== "Microsoft Internet Explorer") {
        ...
        if (trim(document.getElementById("hdsviewgrid").innerHTML) != "") {
            ...
            for (var k = 0; k < klen; k++) {
                ...
                slabno[k] = document.getElementById('hdnexecslab_no_' + k).value;
        ...
        createhtml.append( "<td class='btextsmallsize'>")
         ...
         createhtml.append("<input  type='hidden' id='hdnexecslab_no_" + klen + "'>")
    ...
    for (var k = 0; k < klen; k++) {
        ...
        document.getElementById('hdnexecslab_no_' + k).value=slabno[k] 
...
function fillagency(event, id) {
    ...
    if (event.keyCode == 113) {
        ...
        Execopoup.bindagencyname(document.getElementById('hiddencompcode').value, "", document.getElementById('hiddenuserid').value, document.getElementById('hiddendateformat').value, "", "", "", "", "", bindreport_callback);
...
function fillvalueingrid(id) {
    ...
    if (document.getElementById('check_' + id).checked == true) {
        ...
        var report = Execopoup.reportname_agency(document.getElementById('hiddencompcode').value, document.getElementById('hdnagency_' + id).value);
        ...
        document.getElementById('hiddenretcode').value = document.getElementById('hiddenretcode').value;
        ...
        document.getElementById('txteffectivefrom').value = ""
        document.getElementById('txteffectiveto').value = ""
        document.getElementById('hiddenretcode').value = ""
...
//        }
       ...
       // }
            ...
            var items = Execopoup.duplicasy(compcode, execcode, agencycode, fromdate, todate, dateformat);
        ...
        if (ds.Tables[0].Rows[0].REC_COUNT >= 1) {
            ...
            //for (var i = 0; i <= gridlen; i++) {
                ...
                Execopoup.savedata(compcode, agencycode, days, limit, recovery, fromdate, todate, execcode, createdby, createddt, updatedby, updateddt, dateformat);
...
function excutegrid(res) {
    ...
    if (document.getElementById("hdsviewgrid").innerHTML == "") {
        ...
        for (klen = 0; klen < expcount; klen++) {
            createhtml += "<td class='btextsmallsize'>"
            createhtml += "<input type='checkbox' style='width:10px;text-align:right;' align='left' class='dropdown_large_corr'; onclick=javascript:fillvalueingrid(id);  id='check_" + klen + "'  />"
            createhtml += "</td>"


            createhtml += "<td class='btextsmallsize'>"
            var report = Execopoup.reportname_agency(document.getElementById('hiddencompcode').value, dsmain.Tables[0].Rows[klen].AG_TYPE_CODE);
            if (report.value.Tables[0].Rows.length > 0) {
                var exec_name = report.value.Tables[0].Rows[0].AGENCY_NAME;
            ...
            createhtml += "<input  type='hidden' id='hdnexecslab_no_" + klen + "' value='" + fndnull(dsmain.Tables[0].Rows[klen].EXEC_SLAB_SNO) + "'>";
            createhtml += "</td>"
...
function mdy_save() {
    ...
    for (var i = 0; i <= gridlen; i++) {
        ...
        if (document.getElementById('check_' + i).checked == true) {
            ...
            var slab_no = document.getElementById('hdnexecslab_no_' + i).value;
            var dateformat = document.getElementById('hiddendateformat').value;

            Execopoup.modify_save(compcode, agencycode, days, limit, recovery, fromdate, todate, execcode, createdby, createddt, updatedby, updateddt, dateformat, slab_no)
        }
//        else {
...
function datadel() {
    for (var i = 0; i <= gridlen; i++) {
        if (document.getElementById('check_' + i).checked == true) {
            ...
            Execopoup.del_dta(compcode, execcode, slab_no)
        }
    }
