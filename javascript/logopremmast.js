...
function CancelClick2() {
    ...
    givebuttonpermission('logopremmast');
...
function SaveClick2(compcode1)
 {
    ...
    var premium = document.getElementById('drpremium').value;
      if ((document.getElementById('txtPosTypCode').value == "") && (document.getElementById('hiddenauto').value != 1)) {
        alert("Please Enter Logo Premium  Type Code");
        document.getElementById('txtPosTypCode').focus();
        return false;
    }
    else {
    ...
    if (document.getElementById('txtPosType').value == "")
     {
        alert("Please Enter Logo Premium Type");
        document.getElementById('txtPosType').focus();
        return false;
    }
    else {
    ...
    if (flag == 1) {
       ...
       // logopremmast.insert(compcode, PosTypCode, PosType, Alias, Amount, premium, userid, fdate, tdate, description, adcat);
       // logopremmast.modify(PosTypCode, PosType, Alias, Amount, premium, compcode, userid, fdate, tdate, description,adcat);
          logopremmast.modify(compcode, PosTypCode, PosType, Alias, Amount, premium, userid, fdate, tdate, description, adcat);
        document.getElementById('btnNew').disabled = true;
        document.getElementById('btnSave').disabled = true;
        ...
        advpostypeds.Tables[0].Rows[z].LOGP_TYPE_CODE = PosTypCode;
        advpostypeds.Tables[0].Rows[z].LOGP_TYPE = PosType;
        advpostypeds.Tables[0].Rows[z].LOGP_TYPE_ALIAS = Alias;
        ...
        var reschk = logopremmast.chkpastypename(PosTypCode, PosType, compcode, userid, flag, fdate, tdate);
        var ds = reschk.value;
        if (ds.Tables[0].Rows.length > 0) {
            alert("This Ad Logo premium Type has already assigned !! ");
            document.getElementById('txtPosType').focus();
            return false;
        }
        logopremmast.chksave(PosTypCode, compcode, userid, callsave);
        return false;
    }
    setButtonImages();
}
    ...
    if (ds.Tables[0].Rows.length == 0) {
        ...
        if (browser != "Microsoft Internet Explorer") {
         ...
         //   httpRequest.onreadystatechange = function() { alertContents(httpRequest) };
            ...
            httpRequest.open("GET", "savelogopoppup.aspx?logoprem=" + PosTypCode, false);
            httpRequest.send('');
            ...
            var xml = new ActiveXObject("Microsoft.XMLHTTP");
            xml.Open("GET", "savelogopoppup.aspx?logoprem=" + PosTypCode, false);
            xml.Send();
            var dl = xml.responseText;
        }
 
       // PosTypCode, PosType, Alias, Amount, premium, compcode, userid, fdate, tdate, description, adcat
        logopremmast.insert(compcode, PosTypCode, PosType, Alias, Amount, premium, userid, fdate, tdate, description, adcat);
        ...
        alert("This Logo Premium Type Code Already Exist");
        document.getElementById('txtPosTypCode').value = "";
...
function uppercase3() {
    ...
    if (lstr.indexOf(" ") == 1) {
        ...
        str = mstr;
        logopremmast.chkadvposition(str, fillcall);
        return false;
    }
    return false;
    ...
    var newstr;
    {
        if (ds.Tables[0].Rows.length == 0) {
           // newstr = null;
            ...
            newstr = ds.Tables[0].Rows[0].DES_TYPE_CODE;
        }
        if (newstr != null) {
...
function ExecuteClick2(compcode1) {
    ...
    logopremmast.Selectpage(compcode, PosTypCode, PosType, Alias, Amount, premium, userid, dateformat, call_Execute);
    //logopremmast.Selectpage(PosTypCode, PosType, Alias, Amount, premium, compcode, userid, call_Execute);
...
function call_Execute(response) {
    ...
    if (advpostypeds.Tables[0].Rows.length > 0) {
        document.getElementById('txtPosTypCode').value = advpostypeds.Tables[0].Rows[0].LOGP_TYPE_CODE;
        document.getElementById('txtPosType').value = advpostypeds.Tables[0].Rows[0].LOGP_TYPE;
        document.getElementById('txtAlias').value = advpostypeds.Tables[0].Rows[0].LOGP_TYPE_ALIAS;
        /*change ankur*/
...
function FirstClick2() {
    ...
    if (advpostypeds.Tables[0].Rows[0].IMAGE_NAME == "" || advpostypeds.Tables[0].Rows[0].IMAGE_NAME == null) {
        document.getElementById('txtdessigndesc').value = "";
    }
    else {

        document.getElementById('txtdessigndesc').value = advpostypeds.Tables[0].Rows[0].IMAGE_NAME;
    }
...
function NextClick2() {
    ...
    if (z != -1 && z >= 0) {
        if (z <= y - 1) {
            document.getElementById('txtPosTypCode').value = advpostypeds.Tables[0].Rows[z].LOGP_TYPE_CODE;
            document.getElementById('txtPosType').value = advpostypeds.Tables[0].Rows[z].LOGP_TYPE;
            document.getElementById('txtAlias').value = advpostypeds.Tables[0].Rows[z].LOGP_TYPE_ALIAS;
            ...
            if (advpostypeds.Tables[0].Rows[z].IMAGE_NAME == "" || advpostypeds.Tables[0].Rows[z].IMAGE_NAME == null) {
                document.getElementById('txtdessigndesc').value = "";
            }
            else {

                document.getElementById('txtdessigndesc').value = advpostypeds.Tables[0].Rows[z].IMAGE_NAME;
            }
...
function DeleteClick2(compcode1) {
    ...
    if (boolReturn == 1) {
        ...
        logopremmast.delete1(PosTypCode, compcode, userid);
        if (browser != "Microsoft Internet Explorer") {
        ...
        logopremmast.Selectpage(glpostypcode, glpostype, glalias, glamount, glpremium, compcode, userid, call_delete);
...
function commission()
{
    ...
    if (popup == "1") {
        ...
        if (document.getElementById('txtPosType').value == "") {
            alert("Please Fill Logo Premium Type");
            document.getElementById('txtPosType').focus();
            return false;
        }
    ...
    var strReturn = window.open('logo_Premium.aspx?' + '&logocode=' + logocode + '&temp=' + temp + '&flag2=' + flag2 + '&ankit=' + ankit, '', 'status=0,toolbar=0,resizable=0,scrollbars=yes,top=144,left=210,width=700px,height=300px');
