...
function showcreditdetail() {
    ...
    var ag = cash_recipt.GETCASH_PAY(document.getElementById('hiddencompcode').value, document.getElementById('drppaymode').value);
    var ds_n = ag.value;
    var cashdiscount = 'N';
    ...
    if (document.getElementById('drppaymode').value == "CR0")  //CR0 is Credit Card code
    {
    ...
    else if (document.getElementById('drppaymode').value == "CH0" || document.getElementById('drppaymode').value == "DD0" || document.getElementById('drppaymode').value == 'PO0' || document.getElementById('drppaymode').value == 'AD0')  //CR0 is Credit Card code and DD0 is demand draft
    {
...
function updatercpt(temp) {
    var kk = 0;
    var bookid = document.getElementById('txtbookingid').value;
    ...
    ro = cash_recipt.updatero(document.getElementById('hiddencompcode').value, document.getElementById('drppaymode').value, bookid, temp, crdmnth, crdyear, crdname, crdtype, crdnumber, chkno, chkdate, chkamt, chkbankname);
...
{
    var res = cash_recipt.insertcashdetail(document.getElementById('txtbookingid').value, document.getElementById('txtbank').value);
    if(res.error!=null)
    {
    ...
    updatercpt(res.value.Tables[0].Rows[0].RECPTNO);
...
function executeclick()
{
 if(document.getElementById('txtbookingid').value=="")
    ...
    var res = cash_recipt.checkreci(cioid, compcode, userid);
    ds=res.value;
    if(ds.Tables[0].Rows.length>0)
        {
        if((ds.Tables[0].Rows[0].Bill_pay=="CR0")||(ds.Tables[0].Rows[0].Bill_pay=="CR4")||(ds.Tables[0].Rows[0].Bill_pay=="CA0" && ds.Tables[0].Rows[0].ro_status=="2"))
       // if (ds.Tables[0].Rows[0].ro_status == "2")
        {
        
        cash_recipt.execute(cioid,document.getElementById('hiddencompcode').value,'',document.getElementById('hiddendateformat').value,exec_callback);    
        document.getElementById('btnmodify').disabled=false;
        return false;
        }
        else
        {
            if (ds.Tables[0].Rows[0].Bill_pay != "CA0" || ds.Tables[0].Rows[0].Bill_pay == "CR4")  //CR4 use for credit_debit_card
            {
        ...
        {
          alert("Please Enter Right Booking Id OR You have no Permission to Confirm That Booking ID");
          cleardata();
          document.getElementById('btnmodify').disabled=true;
          document.getElementById('txtbookingid').value="";
          document.getElementById('txtbookingid').focus();
...
function exec_callback(response)
{
    ...
    if(ds!=null) {
        ...
        if(len!="0") {
            ...
            if (ds.Tables[0].Rows[0].PayMode == "CR4" || ds.Tables[0].Rows[0].PayMode == "CREDIT_DEBIT_CARD") {
                if (ds.Tables[0].Rows[0].BANKNAME != null) {
                if (document.getElementById('txtbank').value == "" || document.getElementById('txtbank').value == "0" || document.getElementById('txtbank').value == null) {
                    ...
                    document.getElementById('txtbank').disabled = false;
                    document.getElementById('txtbank').value = ds.Tables[0].Rows[0].BANKNAME;
                }
                }
            }
            else {
            ...
            else
            {
                if(ds.Tables[0].Rows[0].Package_code.indexOf(",")>0)
                {
                    ...
                    {
                        var ds_pkg=cash_recipt.getPackageName(document.getElementById('hiddencompcode').value,s1[k_]);
                ...
                {
                    var ds_pkg=cash_recipt.getPackageName(document.getElementById('hiddencompcode').value,ds.Tables[0].Rows[0].Package_code);
                    if(ds_pkg.value!=null)
                    {
...
//            }
   ...
   //document.getElementById('txtschemetype').value=ds.Tables[0].Rows[0].Contract_name;
            ...
            /////////////
            if (ds.Tables[0].Rows[0].BANKNAME == null || ds.Tables[0].Rows[0].BANKNAME == "") {
...
function printreceipt(receiptno)
{
    ...
    if (receiptno == null) {
     ...
     var res=cash_recipt.getcioid(recptno);
     var ds =res.value;
    if(ds != null)
    {
      if(ds.Tables[0].Rows.length > 0)
