...
function forCancel() {
    ...
    document.getElementById('drpprodcat').value = "0";
    document.getElementById('drpprodsubcat').value = "0";
...
function bind_edition13() {
    ...
    var pub_cent = "";
    ad_cat_prodcatwise.edition_bind(publication, pub_cent, comp_code, call_bind_edition);
...
//            ad_cat_prodcatwise.fillF2_client(compcode, client, bindclient_callback);
//        }
//    }
...
//            ad_cat_prodcatwise.fillF2_Agency(compcode, agn, bindAgency_callback);
//        }
//    }
...
function bindadcat() {
    ...
    ad_cat_prodcatwise.adcatbnd(adtype1, compcode, call_adcatbind);
...
function bindprodsubcat() {
    var compcode = document.getElementById('hiddencompcode').value;
    var prodsbtype1 = document.getElementById('drpprodcat').value;
    ad_cat_prodcatwise.prodsubcatbnd(prodsbtype1, compcode, call_prodsubcatbind);

}
function call_prodsubcatbind(response) {
    ds = response.value;
    var brand = document.getElementById('drpprodsubcat');
    brand.options.length = 0;
    brand.options[0] = new Option("Select Product SubCat");
    document.getElementById("drpprodsubcat").options.length = 1;


    if (ds.Tables[0].Rows.length > 0) {
        for (var i = 0; i < ds.Tables[0].Rows.length; i++) {
            brand.options[brand.options.length] = new Option(ds.Tables[0].Rows[i].prod_desc, ds.Tables[0].Rows[i].prod_cat_code);
            brand.options.length;
        }
    }
...
//    ad_cat_prodcatwise.fillQBC(document.getElementById('dppubcent').value, bindQBC_callback11);
//}
...
function forreport() {
    ...
    var prodcat = document.getElementById('drpprodcat').value;
    var prodsubcat = document.getElementById('drpprodsubcat').value;
    var reptype = document.getElementById('drpreptype').value;
    var datebasedon = document.getElementById('drpdtbased').value;
    ...
    var branch_nm = "";
    if (document.getElementById("dpbranch").selectedIndex == -1 || document.getElementById("dpbranch").selectedIndex == 0) {
        branch_nm = "";
    }
    else {
        branch_nm = document.getElementById("dpbranch").options[document.getElementById("dpbranch").selectedIndex].text;
    }

    var pub_nm = "";
    if (document.getElementById("dppub1").selectedIndex == -1 || document.getElementById("dppub1").selectedIndex == 0) {
        pub_nm = "";
    }
    else {
        pub_nm = document.getElementById("dppub1").options[document.getElementById("dppub1").selectedIndex].text;
    }

    ad_cat_prodcatwise.setSessionmis_run(fromdate, todate, pcent, branch, pub, edtn, dest, adtype, adcat, prodcat, prodsubcat, reptype,datebasedon, extra1, extra2, extra3, extra4, extra5, adcatNM, branch_nm, pub_nm);
    window.open('ad_cat_prodcatwise_view.aspx');
    return false;
